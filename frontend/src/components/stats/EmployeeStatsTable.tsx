...
const SHIFT_COLS = ['A', 'B', 'C', 'D', 'M', 'OFF', 'SPECIAL'] as const
const SHIFT_COLORS: Record<string, string> = {
  A: 'text-blue-700', B: 'text-orange-700', C: 'text-purple-700',
  D: 'text-indigo-700', M: 'text-teal-700', OFF: 'text-red-700', SPECIAL: 'text-purple-800',
}

export function EmployeeStatsTable({ rows }: { rows: EmployeeStats[] }) {
  return (
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-100 text-gray-600">
          <tr>
            ...
            {SHIFT_COLS.map((s) => (
              <th key={s} className="px-2 py-2 text-right">{s === 'OFF' ? '休' : s === 'SPECIAL' ? '特休' : s}</th>
            ))}
        ...
        <tbody>
          {rows.map((e) => (
            <tr key={e.employee_id} className="border-t border-gray-100 hover:bg-indigo-50/40">
              ...
              <td className={cn('px-2 py-1.5 text-right', e.consecutive_off_count >= 2 ? 'font-bold text-orange-600' : 'text-gray-600')}>
